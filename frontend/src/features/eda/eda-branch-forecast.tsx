...
type LagAssociation = { lag: number; observations: number; correlation: number | null };
type NetworkLink = { branch_a: string; branch_b: string; region_a: string; region_b: string; region_scope: string; overlap_months: number; same_month_correlation: number; strongest_lead_direction: string; strongest_lead_lag: number; strongest_lead_correlation: number | null };
type RegionInfluence = { branch: string; region: string; branch_share_of_region: number | null; correlations: LagAssociation[]; strongest_lag: number; strongest_correlation: number | null };
type SkuInfluence = { base_sku: string; sku_name: string; net_quantity: number; absolute_quantity_share: number; same_month_remainder_correlation: number | null; strongest_lag: number; strongest_lag_correlation: number | null; lag_correlations: LagAssociation[] };
type GlobalSkuInfluence = { base_sku: string; sku_name: string; net_quantity: number; absolute_quantity_share: number; branches: number; top5_branches: number };
type SeasonalComparisonPoint = { month_number: number; selected_index: number | null; selected_observations: number; peer_average_index: number | null; peer_median_index: number | null; peer_branch_count: number; region_total_index: number | null };
type SeasonalBranch = { branch: string; branch_name: string; region: string; seasonal_strength: number | null; selected_similarity: number | null; peak_month: number | null; peak_index: number | null; trough_month: number | null; trough_index: number | null; reliable_months: number; seasonal_gain: number | null; rainy_mean_quantity: number | null; dry_mean_quantity: number | null; rainy_uplift: number | null; profile: ProfilePoint[] };
type Seasonality = { region: string; selected_branch: string; same_region_branch_count: number; comparison_profile: SeasonalComparisonPoint[]; branches: SeasonalBranch[]; evidence_warning: string };
type Data = {
  data_from: string;
  ...
  selected_lag_associations: LagAssociation[];
  branch_lag_profiles: Array<{ branch: string; branch_name: string; region: string; lags: LagAssociation[] }>;
  branch_region_influences: RegionInfluence[];
  selected_region_influence: RegionInfluence;
  selected_sku_influence: SkuInfluence[];
  global_sku_influence: GlobalSkuInfluence[];
...
function seasonalHeatColor(value: number | null | undefined): string {
  if (value === null || value === undefined) return "transparent";
  const difference = value - 1;
  const alpha = .08 + Math.min(Math.abs(difference), .8) * .9;
  return difference >= 0 ? `rgba(167,139,250,${alpha})` : `rgba(35,175,255,${alpha})`;
}
    ...
    <div className="bf-page">
      ...
      <section className="bf-grid">
        <article className="bf-card wide">
          <header><h4>Độ nhớ lag của quantity theo Chi nhánh</h4><p>Corr giữa quantity tháng hiện tại với lag 1/2/3/6/12. Dùng để quyết định cửa sổ lag, không tự động coi corr cao là feature tốt.</p></header>
          <div className="bf-heat-wrap"><table className="bf-heat"><thead><tr><th>Chi nhánh</th>{[1,2,3,6,12].map((lag) => <th key={lag}>Lag {lag}</th>)}</tr></thead><tbody>{data?.branch_lag_profiles.map((row) => <tr key={row.branch}><th>{row.branch}<small>{row.region}</small></th>{row.lags.map((lag) => <td key={lag.lag} style={{ background: heatColor(lag.correlation) }}>{optionalPercent(lag.correlation)}<small>n={lag.observations}</small></td>)}</tr>)}</tbody></table></div>
        </article>

        <article className="bf-card">
          <header><h4>Lag phù hợp cho {data?.selected.branch}</h4><p>Lag 12 có ít origin hơn do lịch sử chỉ từ 2024, cần kiểm tra backtest trước khi dùng.</p></header>
          <div className="bf-lag-list">{data?.selected_lag_associations.map((row) => <div key={row.lag}><b>Lag {row.lag}</b><i><em className={(row.correlation ?? 0) >= 0 ? "positive" : "negative"} style={{ width: `${Math.min(Math.abs(row.correlation ?? 0) * 100,100)}%` }}/></i><strong>{optionalPercent(row.correlation)}</strong><small>{row.observations} origin</small></div>)}</div>
        </article>

        <article className="bf-card">
          <header><h4>Ảnh hưởng phần còn lại của Vùng</h4><p>Đã loại {data?.selected.branch} khỏi tổng vùng để tránh chính nó làm correlation tăng giả.</p></header>
          <div className="bf-region-summary"><strong>{data?.selected_region_influence.region}</strong><span>Share CN trong vùng: {optionalPercent(data?.selected_region_influence.branch_share_of_region)}</span>{data?.selected_region_influence.correlations.map((row) => <div key={row.lag}><b>{row.lag === 0 ? "Cùng tháng" : `Vùng lag ${row.lag}`}</b><em style={{ color: (row.correlation ?? 0) >= 0 ? "#16d8c2" : "#ff7373" }}>{optionalPercent(row.correlation)}</em><small>n={row.observations}</small></div>)}</div>
        </article>

        <article className="bf-card wide">
          <header><h4>So sánh mức phụ thuộc Vùng giữa các Chi nhánh</h4><p>Mỗi ô là corr của tổng các CN còn lại trong cùng vùng với quantity CN đang xét. “Chưa xác định” là cảnh báo cần chuẩn hóa mapping vùng.</p></header>
          <div className="bf-data-table"><table><thead><tr><th>Chi nhánh</th><th>Vùng</th><th>Share trong vùng</th><th>Cùng tháng</th><th>Vùng lag 1</th><th>Vùng lag 2</th><th>Vùng lag 3</th><th>Tín hiệu mạnh nhất</th></tr></thead><tbody>{data?.branch_region_influences.map((row) => <tr key={row.branch}><td><b>{row.branch}</b></td><td>{row.region}</td><td>{optionalPercent(row.branch_share_of_region)}</td>{[0,1,2,3].map((lag) => { const value=row.correlations.find((item) => item.lag === lag)?.correlation; return <td key={lag} style={{ background: heatColor(value) }}>{optionalPercent(value)}</td>; })}<td>{row.strongest_lag === 0 ? "Cùng tháng" : `Lag ${row.strongest_lag}`} · {optionalPercent(row.strongest_correlation)}</td></tr>)}</tbody></table></div>
        </article>

        <article className="bf-card wide">
          <header><h4>Base SKU ảnh hưởng nhiều nhất tới {data?.selected.branch}</h4><p>Share dùng absolute quantity; corr cùng tháng so với phần còn lại của CN; lag đo SKU quá khứ so với tổng CN hiện tại.</p></header>
          <div className="bf-data-table"><table><thead><tr><th>Base SKU</th><th>Net quantity</th><th>Absolute share</th><th>Corr với phần còn lại</th><th>Lag mạnh nhất</th><th>Corr lag</th></tr></thead><tbody>{data?.selected_sku_influence.slice(0,20).map((row) => <tr key={row.base_sku}><td><b>{row.base_sku}</b><small>{row.sku_name || "—"}</small></td><td>{formatNumber(row.net_quantity)}</td><td>{formatPercent(row.absolute_quantity_share)}</td><td>{optionalPercent(row.same_month_remainder_correlation)}</td><td>Lag {row.strongest_lag}</td><td>{optionalPercent(row.strongest_lag_correlation)}</td></tr>)}</tbody></table></div>
        </article>

        <article className="bf-card wide">
          <header><h4>Base SKU có ảnh hưởng rộng trên toàn hệ thống CN active</h4><p>Xếp theo absolute quantity 12 tháng; số CN Top-5 cho biết SKU thường xuyên nằm trong nhóm kéo quantity của chi nhánh.</p></header>
          <div className="bf-data-table"><table><thead><tr><th>Base SKU</th><th>Net quantity</th><th>Share toàn hệ thống</th><th>Số CN có bán</th><th>Số CN thuộc Top-5</th></tr></thead><tbody>{data?.global_sku_influence.slice(0,20).map((row) => <tr key={row.base_sku}><td><b>{row.base_sku}</b><small>{row.sku_name || "—"}</small></td><td>{formatNumber(row.net_quantity)}</td><td>{formatPercent(row.absolute_quantity_share)}</td><td>{row.branches}</td><td>{row.top5_branches}</td></tr>)}</tbody></table></div>
        </article>
      </section>
      ...
      <section className="bf-grid bf-seasonality">
        <article className="bf-card wide">
          <header><h4>Mùa vụ của {data?.selected.branch} so với các CN cùng vùng {data?.seasonality.region}</h4><p>Index 1,0 là mức trung bình. “TB ngang CN” cho mỗi chi nhánh trọng số bằng nhau; “Tổng vùng” có trọng số theo quy mô.</p></header>
          <div className="bf-season-kpis"><span>CN cùng vùng <b>{formatNumber(data?.seasonality.same_region_branch_count)}</b></span><span>Peak <b>{data?.seasonality.branches[0]?.peak_month ? MONTHS[data.seasonality.branches[0].peak_month - 1] : "—"}</b></span><span>Đáy <b>{data?.seasonality.branches[0]?.trough_month ? MONTHS[data.seasonality.branches[0].trough_month - 1] : "—"}</b></span><span>Mùa mưa uplift <b>{optionalPercent(data?.seasonality.branches[0]?.rainy_uplift)}</b></span></div>
          <div className="bf-chart"><ResponsiveContainer width="100%" height="100%"><ComposedChart data={data?.seasonality.comparison_profile ?? []} margin={{ top: 12, right: 20, bottom: 4, left: 4 }}><CartesianGrid stroke="#1d3547" strokeDasharray="3 3"/><XAxis dataKey="month_number" tickFormatter={(value: number) => MONTHS[value - 1]}/><YAxis domain={["auto", "auto"]} tickFormatter={(value: number) => value.toFixed(1)}/><ReferenceLine y={1} stroke="#ffc14d" strokeDasharray="4 3"/><Tooltip formatter={(value: unknown, name: unknown) => [Number(value).toFixed(2), String(name)]}/><Legend/><Line dataKey="selected_index" name={data?.selected.branch ?? "CN đã chọn"} stroke="#16d8c2" strokeWidth={3} connectNulls/><Line dataKey="peer_average_index" name="TB ngang CN cùng vùng" stroke="#a78bfa" strokeWidth={2} dot={false} connectNulls/><Line dataKey="region_total_index" name="Tổng vùng" stroke="#23afff" strokeDasharray="5 3" dot={false} connectNulls/></ComposedChart></ResponsiveContainer></div>
          <small className="bf-warning">{data?.seasonality.evidence_warning}</small>
        </article>

        <article className="bf-card wide">
          <header><h4>Heatmap mùa vụ các Chi nhánh cùng vùng</h4><p>Tím = cao hơn mean của chính CN, xanh = thấp hơn. Bấm một dòng để xem chi tiết chi nhánh đó.</p></header>
          <div className="bf-heat-wrap"><table className="bf-heat bf-season-heat"><thead><tr><th>Chi nhánh</th>{MONTHS.map((month) => <th key={month}>{month}</th>)}<th>Độ mạnh</th><th>Giống CN chọn</th></tr></thead><tbody>{data?.seasonality.branches.map((row) => <tr key={row.branch} className={row.branch === data.selected.branch ? "selected" : ""} onClick={() => changeBranch(row.branch)}><th><button type="button" onClick={() => changeBranch(row.branch)}>{row.branch}<small>{row.branch_name}</small></button></th>{row.profile.map((point) => <td key={point.month_number} style={{ background: seasonalHeatColor(point.seasonal_index) }} title={`${MONTHS[point.month_number - 1]}: ${point.seasonal_index?.toFixed(2) ?? "—"}; n=${point.observations}`}>{point.seasonal_index?.toFixed(2) ?? "—"}<small>n={point.observations}</small></td>)}<td>{optionalPercent(row.seasonal_strength)}</td><td>{optionalPercent(row.selected_similarity)}</td></tr>)}</tbody></table></div>
        </article>

        <article className="bf-card wide">
          <header><h4>Xếp hạng mùa vụ trong vùng {data?.seasonality.region}</h4><p>Độ mạnh đo độ lệch profile khỏi 1,0. “Reliable months” đếm số tháng có ít nhất hai năm quan sát.</p></header>
          <div className="bf-data-table"><table><thead><tr><th>Chi nhánh</th><th>Peak</th><th>Index peak</th><th>Đáy</th><th>Index đáy</th><th>Độ mạnh</th><th>Seasonal gain</th><th>Mùa mưa uplift</th><th>Reliable months</th></tr></thead><tbody>{data?.seasonality.branches.toSorted((a,b) => (b.seasonal_strength ?? -1) - (a.seasonal_strength ?? -1)).map((row) => <tr key={row.branch} className={row.branch === data.selected.branch ? "selected" : ""} onClick={() => changeBranch(row.branch)}><td><b>{row.branch}</b><small>{row.branch_name}</small></td><td>{row.peak_month ? MONTHS[row.peak_month - 1] : "—"}</td><td>{row.peak_index?.toFixed(2) ?? "—"}</td><td>{row.trough_month ? MONTHS[row.trough_month - 1] : "—"}</td><td>{row.trough_index?.toFixed(2) ?? "—"}</td><td>{optionalPercent(row.seasonal_strength)}</td><td>{optionalPercent(row.seasonal_gain)}</td><td>{optionalPercent(row.rainy_uplift)}</td><td>{row.reliable_months}/12</td></tr>)}</tbody></table></div>
        </article>
      </section>
      ...
      <section className="bf-grid">
        ...
        <article className="bf-card wide">
          <header><h4>Liên hệ giữa các Chi nhánh</h4><p>Same-month là đồng biến; kiểm tra cả lead–lag 1–3 tháng, nhưng không diễn giải thành quan hệ nhân quả.</p></header>
          <div className="bf-link-grid">{data?.selected_branch_links.map((row) => <div key={`${row.branch_a}-${row.branch_b}`}><b>{row.branch_a} ↔ {row.branch_b}</b><span>Đồng tháng {optionalPercent(row.same_month_correlation)}</span><span>{row.strongest_lead_direction} · lag {row.strongest_lead_lag} · {optionalPercent(row.strongest_lead_correlation)}</span><small>{row.region_scope} ({row.region_a} / {row.region_b}) · {row.overlap_months} tháng giao nhau</small></div>)}</div>
        </article>
      </section>
